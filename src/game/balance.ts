...
// --- Endgame: the googol "win" ----------------------------------------------
// Reaching this held-goo peak (a GOOGOL, 1e100 — a 1 followed by a hundred
// zeros) is the game's victory summit: it fires a one-time champion celebration
// and grants the exclusive champion crown (see cosmetics.ts 'acc-champion').
// Owning that crown IS the persisted "I won" flag, so no extra save field is
// needed. (The bar was raised here from a decillion; save migration v-bump
// strips any crown earned under the old, lower bar — see game/save.ts.)
export const googolWinGoo = 1e100;
