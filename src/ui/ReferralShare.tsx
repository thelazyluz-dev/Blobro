...
import { referralFriendsForGoldMedal, referralFriendsForMedal } from '../game/balance';
...
export function ReferralShare() {
  ...
  const tier = (friends: number, icon: string, label: string, reward: string) => {
    ...
    return (
      <div className={`flex items-center gap-2 rounded-xl px-3 py-2 ring-1 ${done ? 'bg-cy/15 ring-cy/40' : 'bg-black/25 ring-hairline'}`}>
        <span className="text-2xl">{done ? icon : '🔒'}</span>
        <div className="flex-1 text-start">
          <div className={`font-display text-sm ${done ? 'text-cy' : 'text-bone/70'}`}>{label}</div>
          <div className="text-[11px] text-bone/50">{reward}</div>
        </div>
        <span className="shrink-0 text-xs tabular text-bone/60">{friends} 👥</span>
      </div>
    );
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-void/80 p-6" onClick={() => setOpen(false)} role="presentation">
      ...
      >
        ...
        <p className="mx-auto mt-1 max-w-[16rem] text-sm text-bone/70">
        ...
        <div className="mt-4 flex items-center justify-between text-sm">
          <span className="text-bone/60">חֲבֵרִים שֶׁהִצְטָרְפוּ</span>
          <span className="font-display text-cy tabular">
            {count}/{target}
          </span>
        </div>
        ...
        {link ? (
          <div className="mt-4 flex items-center gap-1.5 rounded-full bg-black/40 p-1 ring-1 ring-hairline">
            <input
              ...
              className="min-w-0 flex-1 bg-transparent px-2 text-[11px] text-bone/70 outline-none"
            />
            <button type="button" onClick={onCopy} className="shrink-0 rounded-full bg-black/40 px-3 py-1 text-xs text-bone/80 ring-1 ring-hairline active:scale-95">
              {copied ? '✓' : 'הַעְתֵּק'}
            </button>
          </div>
        ) : (
